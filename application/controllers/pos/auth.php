...
class Auth extends CI_Controller {
	...
	*/
			...
			if ($this->form_validation->run() == true)
			{
				...
				if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
				{
					...
					case 13:
						...
						break;
					default:
						$this->session->set_flashdata('message','<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
	  								 Incorrect Login! </div>');
						redirect('pos/auth/login', 'refresh');
						break;
						}	
				}
				else
				{
					...
					redirect('pos/auth/login', 'refresh'); //use redirects instead of loading views for compatibility with MY_Controller libraries
				}
			}
			else
			{
				...
				$this->data['password'] = array('name' => 'password',
	             ...
	             $this->_render_page('pos/header/header_login');
				 $this->_render_page('pos/auth/login', $this->data);
				 $this->load->view('pos/footer/footer_login');
			}

		}
		else
		{
			redirect($this->session->flashdata('last_location'));
		}
	}
	...
	function create_user()
	{
		...
		$this->data['form_action'] = 'pos/auth/create_user';
		/*if (!$this->ion_auth->logged_in() || !$this->ion_auth->is_admin())
		{
			redirect('inventory/auth', 'refresh');
		}*/
		...
		if ($this->form_validation->run() == true && $this->ion_auth->register($username, $password, $email,$additional_data,$group))
		{
			...
			redirect("pos/auth/list_users", 'refresh');
		}
		else
		{
			...
			{
				case 1:
				$this->data['options'] = array(''=>'Select group','13'=>'Cashier','12'=>'Admin');
				break;
				case 12:
				$this->data['options'] = array(''=>'Select group','13'=>'Cashier');
				break;
			}
            ...
            $this->_render_page('pos/header/header',$header);
			//$this->_render_page($menu);
			$this->_render_page('pos/menu/menu');
			$this->_render_page('pos/auth/create_user_admin', $this->data);
			$this->_render_page('pos/footer/footer');
		}
	} 

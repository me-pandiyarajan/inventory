...
var invoice = new Bloodhound({
    ...
    remote: base_url + 'pos/invoice/ajaxInvoiceLookup/%QUERY'
});
...
$('.typeahead').bind('typeahead:selected', function(obj, datum, name) {
      ...
      $.getJSON(base_url + "pos/invoice/ajaxInvoiceDetails/"+ datum.id ,function(product){
         ...
         $.each(product.invoicelist, function(index,key){
            ...
            newRow = '<tr>' +
                        '<td ><input type="hidden" name="invoiceid[]" value="'+ key.InvoiceId +'" /><input type="hidden" name="product_ids[]" value="'+ p_id +'" /><span class="glyphicon glyphicon-trash remover"></span></td>' +
                        '<td >'+ description +' <input type="hidden"  id="descriptions" name="descriptions[]" value="'+ description +'" /> </td>' +
                        '<td ><input type="text" autocomplete="off" max="'+ key.Quantity +'" class="form-control quantity" id="q'+ p_id +'" name="quantities[]" pd-id="'+ p_id +'" quantity-unit="'+ key.Unit +'" actual-quantity="'+ key.availableQuantity +'" ssl="'+ key.SafetyStockLevel +'" value="'+ key.Quantity +'" /> </td>' +
                        '<td >'+ key.Unit +' <input type="hidden" id="unit" name="units[]" value="'+ key.Unit +'" /></td>' +
                        '<td ><i class="fa fa-inr"></i> '+ key.UnitPrice +'<input type="hidden" id="p'+ p_id +'" name="prices[]" value="'+ key.UnitPrice +'" /> </td>' +
                        ...
                        '<td ><i class="fa fa-inr"></i> <span>'+ key.Discount +'</span><input type="hidden" id="dp'+ p_id +'" pd-id="'+ p_id +'"  class="discount_amounts" name="discount_percents[]" value="'+ key.Discount +' " /><input type="hidden" id="dpr'+ p_id +'" pd-id="'+ p_id +'" name="discount_prices[]" value="'+ key.Discount +' " /></td>' +
...
*/
    ...
    */
    ...
    $( "table#return_sale" ).on( "blur",".quantity", function() {
